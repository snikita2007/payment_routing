...
Столбцы таблицы распределения:

| Столбец | Пояснение |
| --- | --- |
| `шт` | сколько заявок ушло провайдеру |
| `факт%` | его доля по числу заявок |
| `цель%` (первый) | целевая доля по числу заявок — `traffic_percentage` провайдера |
| `Δ` (первый) | факт минус цель по числу, в п.п.: `+` — перебор, `−` — недобор |
| `объём%` | доля по сумме заявок |
| `цель%` (второй) | целевая доля по объёму — `volume_share_pct` |
| `Δ` (второй) | то же отклонение, но по объёму |

Прочерк `—` вместо цели и Δ значит, что цель у провайдера не задана; это не то же самое, что
цель 0. Строка `итого` — сумма столбца `шт`.
